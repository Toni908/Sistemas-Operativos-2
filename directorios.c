...
int buscar_entrada(const char *camino_parcial, unsigned int *p_inodo_dir, unsigned int *p_inodo, unsigned int *p_entrada, char reservar, unsigned char permisos) {
    ...
    // Calcular número de entradas, para saber hasta dónde iterar
    cant_entradas_inodo = inodo_dir.tamEnBytesLog / sizeof(struct entrada);

    // --- INICIO OPTIMIZACIÓN CON BUFFER ---
    ...
    while (num_entrada_inodo < cant_entradas_inodo) {
        // Leemos el siguiente bloque del disco si es necesario
        ...
        // Cargamos la entrada del buffer en memoria
        entrada = buffer_lectura[num_entrada_inodo % entradas_por_bloque];

        // Si encontramos la entrada, salimos del bucle (la variable 'entrada' ya la tiene)
        if (strcmp(inicial, entrada.nombre) == 0) break;
        
        num_entrada_inodo++;
    }
    // --- FIN OPTIMIZACIÓN CON BUFFER ---
